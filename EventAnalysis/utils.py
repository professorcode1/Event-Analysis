...
def create_grid_metadata_arrays(date_index: pd.DatetimeIndex, rain_event_matrix:np.ndarray):
    ...
    date_index_array = np.asarray(list(map(lambda x:x.seconds, ((date_index - starting_date) // 3600))))
    ...
    return np.asarray(grid_to_event_map_py), np.asarray(grdPnt_numOfEvent), maximum_number_of_events
